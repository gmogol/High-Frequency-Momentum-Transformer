...
class ModelFeatures:
    ...
    def __init__(
        ...
        df,
        total_time_steps,
        start_boundary='2020-01-09 09:30:00',
        test_boundary='2020-01-09 09:45:00',
        test_end='2020-01-09 09:50:00',
        ...
        self._column_definition = [
            ...
            ("log_1min", DataTypes.REAL_VALUED, InputTypes.TARGET),
            ("log_10min", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("log_30min", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("log_1h", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("log_3h", DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
          #  ("volatility_log_1min",DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("volatility_log_10min",DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("volatility_log_30min",DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("volatility_log_1h",DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
            ("volatility_log_3h",DataTypes.REAL_VALUED, InputTypes.KNOWN_INPUT),
                ...
                def _batch_single_entity(input_data):
                    ...
                    if additional_time_steps_required > 0:
                        if x.dtype == np.dtype('<M8[ns]'):
                             x = np.concatenate(
                            [x, np.full((additional_time_steps_required, x.shape[1]),fill_value=x[-1])]
                        )
...
# aapl=pd.read_csv('data/trades_AAPL_20200101-20200110.csv',nrows=50_000)
# aapl['Time']=pd.to_datetime(aapl['Time'])
# aapl.set_index(aapl.Time,inplace=True)

# aapl_market = drop_closed_market(aapl)
# aapl_market= aapl_market[['Time','Trade_Volume','Trade_Price','Symbol']]
# aapl_market.rename(columns={'Symbol':'ticker'},inplace=True)
...
#     calc_log_return(aapl_market,t)

# df=pd.read_csv('../prepocessed_data/jpm_msft.csv',nrows=1_000_00)    
# test = ModelFeatures(df,50)
